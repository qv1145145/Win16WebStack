...
/* ================================================================
 *  辅助函数 1：处理 smooth hinting 中的 serif 和基础圆角
 * ================================================================ */
static FT_Pos
af_latin_smooth_adjust_base( FT_Pos      dist,
                             FT_UInt     base_flags,
                             FT_UInt     stem_flags,
                             FT_Int      vertical )
{
 ...
 * ================================================================ */
static FT_Pos
af_latin_smooth_compare_standard( AF_LatinAxis  axis,
                                  FT_Pos        dist,
                                  FT_Pos        base_delta,
                                  FT_Pos        org_dist,
                                  FT_UInt       ppem )
{
    ...
    else
    {
        ...
        if ( ( ( org_dist > 0 ) && ( base_delta > 0 ) ) ||
             ( ( org_dist < 0 ) && ( base_delta < 0 ) ) )
        {
            if ( ppem < 10 )
                bdelta = base_delta;
            else if ( ppem < 30 )
...
/* ================================================================
 *  辅助函数 3：strong hinting 的垂直/水平处理
 * ================================================================ */
static FT_Pos
af_latin_strong_adjust( AF_LatinAxis  axis,
                        FT_Pos        dist,
                        FT_Pos        org_dist,
                        FT_Int        vertical,
                        AF_GlyphHints hints )
...
/* ================================================================
 *  主函数：af_latin_compute_stem_width
 * ================================================================ */
static FT_Pos
                             ...
                             FT_UInt        stem_flags )
{
    ...
    {
        /* smooth hinting */
        dist = af_latin_smooth_adjust_base( dist, base_flags, stem_flags, vertical );

        if ( axis->width_count > 0 )
            dist = af_latin_smooth_compare_standard( axis, dist, base_delta, width, ppem );
    }
    else
    {
        /* strong hinting */
        ...
        dist = af_latin_strong_adjust( axis, dist, org_dist, vertical, hints );
    }
