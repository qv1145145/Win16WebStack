...
#define TRICK_SFNT_ID_prep  2
                ...
                FT_Parameter*  params )
  {
    ...
    SFNT_Service  sfnt;
    TT_Face       face = (TT_Face)ttface;
    FT_Driver			driver_ptr;
    

    FT_TRACE2(( "TTF driver\n" ));

		library = ttface->driver->root.library;  
		//if ( IsBadReadPtr( ttface->driver->root.library, sizeof(FT_Library) ) )
			// LOG("tt_face_init", "ttface->driver->root.library is BAD READ! ");
		// else
			// LOG("tt_face_init", "ttface->driver->root.library can read");
    /*hmemcpy( (void __huge *)&driver_ptr, 
    				 (const void __huge *)&ttface->driver, 
    				 sizeof( FT_Driver ) );
   	hmemcpy( (void __huge *)&library, 
   					 (const void __huge *)&ttface->driver, 
   					 sizeof( FT_Library ) );*/
    
    // LOG("will into FT_Get_Module_Interface", "DEBUG");
    sfnt = (SFNT_Service)FT_Get_Module_Interface( library, "sfnt" );
    // LOG("FT_Get_Module_Interface done", "DEBUG");
    if ( !sfnt )
    {
    ...
    FT_TRACE2(( "  " ));
    // LOG("will into sfnt->init_face", "DEBUG");
		error = sfnt->init_face( stream, face, face_index, num_params, params );  
		// LOG("tt_face_init", "sfnt->init_face = %04X:%04X, sfnt = %04X:%04X", 
		//		FARPTR_SEG( sfnt->init_face ), FARPTR_OFF( sfnt->init_face ), 
		//		FARPTR_SEG( sfnt ), FARPTR_OFF( sfnt ) ); 
    // LOG("tt_face_init", "sfnt->init_face done, and error = %d", error);
    
    /* Stream may have changed. */
    stream = face->root.stream;
    /*hmemcpy( (void __huge *)&stream, 
    				 (const void __huge *)&face->root.stream, 
    				 sizeof( FT_Stream ) );  */
    
    {
    	char buf[64];
    	wsprintf(buf, "face->format_tag = 0x%08lX", face->format_tag);
    	// LOG("tt_face_init", buf);
    }
    
    if ( error )
    {
      // LOG("tt_face_init", "error is true");
      goto Exit;  
    }
    ...
    /* Load font directory */
    // LOG("tt_face_init", "undefine TT_USE_BYTECODE_INTERPRETER, will into sfnt->load_face");
    error = sfnt->load_face( stream, face, face_index, num_params, params );
    // LOG("tt_face_init", "sfnt->load_face done");
    if ( error )
      goto Exit;
    ...
    // LOG("tt_face_init", "will into tt_face_load_hdmx");
         ...
         FT_HAS_SBIX( ttface )    )
    {
...
#endif
      {
        // LOG("tt_face_init", "undefine FT_CONFIT_OPTION_INCREMENTAL, and will into tt_face_load_loca");
      ...
      // LOG("tt_face_init", "will into tt_face_load_cvt");
      error = tt_face_load_cvt( face, stream );         
      // LOG("tt_face_init", "tt_face_load_cvt done");
      if ( error && FT_ERR_NEQ( error, Table_Missing ) )
        goto Exit;

      // LOG("tt_face_init", "will into tt_face_load_fpgm");
      error = tt_face_load_fpgm( face, stream );
      // LOG("tt_face_init", "tt_face_load_fpgm done");
      if ( error && FT_ERR_NEQ( error, Table_Missing ) )
        goto Exit;
      
      // LOG("tt_face_init", "will into tt_face_load_prep");
      error = tt_face_load_prep( face, stream );
      // LOG("tt_face_init", "tt_face_load_prep done");
      if ( error && FT_ERR_NEQ( error, Table_Missing ) )
      {
        // LOG("tt_face_init", "error && FT_ERR_NEQ( error, Table_Missing ) is true");
        goto Exit;
      }
...
#endif
      {
        // LOG("tt_face_init", "undefine FT_CONFIG_OPTION_INCREMENTAL");
             ...
             tt_check_single_notdef( ttface ) )
        {
          ...
          // LOG("tt_face_init", "ttface->num_fixed_sizes          &&"
           //   "face->glyph_locations            &&"
           //   "tt_check_single_notdef( ttface ) ) is true");
...
#ifdef TT_CONFIG_OPTION_GX_VAR_SUPPORT
    {
      ...
      // LOG("tt_face_init", "defined TT_CONFIG_OPTION_GX_VAR_SUPPORT"); 
      if ( instance_index && FT_HAS_MULTIPLE_MASTERS( ttface ) )
      {
        error = FT_Err_Ok; /*FT_Set_Named_Instance( ttface, instance_index );*/
        // LOG("tt_face_init", "instance_index && FT_HAS_MULTIPLE_MASTERS( ttface ) is true, and error = %d", error);
        if ( error )
          goto Exit;
      }
    }
    ...
    // LOG("tt_face_init", "undefine TT_CONFIG_OPTION_GX_VAR_SUPPORT, and will into TT_Init_Glyph_Loading");
    /* initialize standard glyph loading routines */
    TT_Init_Glyph_Loading( face ); 
    // LOG("tt_face_init", "TT_Init_Glyph_Loading done");

  Exit:
    // LOG("tt_face_init", "into Exit:, and error = %d", error);
    return error;

  Bad_Format:
    error = FT_THROW( Unknown_File_Format );
    // LOG("tt_face_init", "into Bad_Format:");
    goto Exit;
  }
