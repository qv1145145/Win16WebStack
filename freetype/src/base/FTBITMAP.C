...
FT_EXPORT_DEF( FT_Error ) __far __pascal
FT_Bitmap_Blit( FT_Library         library,
                const FT_Bitmap   *source,
                FT_Pos             source_offset_x,
                FT_Pos             source_offset_y,
                FT_Bitmap         *target,
                FT_Pos            *target_offset_x,
                FT_Pos            *target_offset_y,
                FT_Color           color )
{
    FT_Vector  source_offset;
    FT_Vector  target_offset;

    source_offset.x = source_offset_x;
    source_offset.y = source_offset_y;
    target_offset.x = target_offset_x ? *target_offset_x : 0;
    target_offset.y = target_offset_y ? *target_offset_y : 0;

    return FT_Bitmap_Blend( library, source, source_offset,
                            target, &target_offset, color );
}
