...
#include <freetype/internal/svgint~1.h>
...
#include <windows.h>  // DEGUG

typedef unsigned long ULONG;

#define _memcpy( dest, src, count ) hmemcpy( (void __huge *)&dest, (const void __huge *)&src, sizeof( count ) );
...
#pragma optimize("", off)
                 ...
                 FT_Stream           *astream )
  {
    ...
    *astream = NULL;
    // LOG("into FT_Stream_New function", "DEBUG");  
    
    {
      char buf[256];
      wsprintf(buf, "&args (stack addr) = %04X:%04X\n"
      			    "args (value)       = %04X:%04X\n"
      			    "args->flags		= %u", 
      	(unsigned int)(((unsigned long)(&args)) >> 16),
      	(unsigned int)(((unsigned long)(&args)) & 0xFFFF),
      	(unsigned int)(((unsigned long)args) >> 16), 
      	(unsigned int)(((unsigned long)args) & 0xFFFF), 
      	args->flags, mode, FT_OPEN_PATHNAME);
      // LOG(buf, "FT_Stream_New: args");
    }
    ...
    if ( mode == FT_OPEN_MEMORY )
    {
      ...
      if ( FT_NEW( stream ) )
        goto Exit;
      // LOG("will into FT_Stream_OpenMemory", "DEBUG");
      FT_Stream_OpenMemory( stream,
                            (const FT_Byte*)args->memory_base,
                            (FT_ULong)args->memory_size );
      // LOG("FT_Stream_OpenMemory done", "DEBUG");                      
    ...
    {
      // LOG("mode==FT_OPEN_PATHNAME is true", "DEBUG");
      /* create a normal system stream */
      if ( FT_NEW( stream ) )
        goto Exit;

      stream->memory = memory;
      // // LOG("Will into FT_Sream_Open function", "DEBUG");
      {
    	char buf[256];
    	wsprintf(buf, "In FT_Stream_New, args->pathname = [%s]", args->pathname ? args->pathname : "(null)");
    	// LOG(buf, "Will into FT_Stream_Open");
    	}
      error = FT_Stream_Open( stream, args->pathname );
      {
      	char buf[64];
      	wsprintf(buf, "FT_Stream_Open returned: 0x%04X", error);
      	// LOG(buf, "DEBUG");
      }
      if ( error )
        FT_FREE( stream );
    ...
    {
      // LOG("(mode==FT_OPEN_STREAM)&&args->stream is true", "DEBUG");
      /* use an existing, user-provided stream */

      /* in this case, we do not need to allocate a new stream object */
      /* since the caller is responsible for closing it himself       */
      stream         = args->stream;                    
      hmemcpy( (void __huge *)&stream->memory, (const void __huge *)&memory, sizeof( FT_Memory) );
      error          = FT_Err_Ok;                                       
    }
    ...
    else
    {
      ...
      {
        // LOG("(args->flags&FT_OPEN_STREAM)&&args->stream is true, \n will into ft_stream_close", "DEBUG");
        FT_Stream_Close( args->stream );   
        // LOG("ft_stream_close done", "DEBUG");
      }
    }

    if ( !error ){
      // LOG("!error is true", "DEBUG");   
      // *astream       = stream;
      hmemcpy( (void __huge *)astream, 
      				 (const void __huge *)&stream, 
      				 sizeof( FT_Stream ) ); 
    }
...
#define FT_COMPONENT  objs
  ...
  find_unicode_charmap( FT_Face  face )
  {
    ...
    // LOG("find_unicode_charmap", "into find_unicode_charmap");
    ...
    for ( ; --cur > first; )
    {
      // LOG("find_unicode_charmap", "into first for loop");
      /*if ( IsBadReadPtr(cur[0]->encoding, sizeof( FT_Encoding ) ) )
      	// LOG("find_unicode_charmap", "cur[0]->encoding is BAD READ!");
      else
      	// LOG("find_unicode_charmap", "cur[0]->encoding can read");
      */	  
            // LOG("find_unicode_charmap", "cur[%d] = %04X:%04X, num_charmaps = %d", 
      		// (int)(cur - first), FARPTR_SEG( cur[0] ), FARPTR_OFF( cur[0] ), face->num_charmaps);
      
      if ( cur[0]->encoding == FT_ENCODING_UNICODE )
      {
        // LOG("find_unicode_charmap", "cur[0]->encoding == FT_ENCODING_UNICODE is true");
        ...
        {
          // LOG("find_unicode_charmap", "( cur[0]->platform_id == ..... is true");
          face->charmap = cur[0];     
          // LOG("find_unicode_charmap", "face->charmap=cur[0] done, and will return FT_Err_Ok!!");
          return FT_Err_Ok;
        }
      }       
    }
    ...
    for ( ; --cur > first; )
    {
      // LOG("find_unicode_charmap", "will decide cur[0]->encoding == FT_ENCODING_UNICODE");
      if ( cur[0]->encoding == FT_ENCODING_UNICODE )
      {
        // LOG("find_unicode_charmap", "cur[0]->encoding == FT_ENCODING_UNICODE is true");
        face->charmap = cur[0];    
             ...
             FT_Face       *aface )
  {
    ...
    face->memory = memory;
    face->stream = *astream;
    // hmemcpy( (void __huge *)&face->stream, (const void __huge *)astream, sizeof( FT_Stream ) );
    ...
    // LOG("open_face", "face->internal = %04X:%04X, sizeof(*internal)=%u", FARPTR_SEG( internal ), FARPTR_OFF( internal ), sizeof( *internal ) );                                                   
    // LOG("open_face", "face->internal->transform_flags = %d", face->internal->transform_flags);
    ...
    if ( clazz->init_face )
    {
      // LOG("clazz->init_face is true \n and will into clazz->init_face(...)", "DEBUG");
      error = clazz->init_face( *astream,
                                face,
                                (FT_Int)face_index,
                                num_params,
                                params );  
			// LOG("open_face", "clazz->init_face returned: %d", error);
    }
    /* Stream may have been changed. */     
    // LOG("open_face", "will *astream = face->stream;");
    *astream = face->stream;   
    // hmemcpy( (void __huge *)astream, (const void __huge *)&face->stream, sizeof( FT_Stream ) );
    // LOG("open_face", "will *anexternal_stream = ( face->face_flags & FT_FACE_FLAG_EXTERNAL_STREAM ) != 0;");
    *anexternal_stream =
      ( face->face_flags & FT_FACE_FLAG_EXTERNAL_STREAM ) != 0;
    if ( error )
      goto Fail;
    ...
    // LOG("open_face", "will into find_unicode_charmap function");
    error2 = find_unicode_charmap( face );                      
    // LOG("open_face", "find_unicode_charmap done, and it returned (error2): %d", error);
    ...
    // LOG("open_face", "will *aface = face");
    *aface = face;

  Fail:   
  	// LOG("open_face", "into Fail:");
    if ( error )
    {
      destroy_charmaps( face, memory );
      if ( clazz->done_face )
...
#define FT_COMPONENT  raccess
    ...
    for ( i = 0; i < FT_RACCESS_N_RULES; i++ )
    {
      ...
      error = FT_Stream_New( library, memory, &args2, &stream2 );
...
#define FT_COMPONENT  objs
  ...
  FT_Open_Face( FT_Library           library,
                ...
                FT_Face             *aface )
  {
  	// LOG("Into FT_Open_Face function.", "DEBUG");
    return ft_open_face_internal( library, args, face_index, aface, 1 );
  }
                         ...
                         FT_Bool              test_mac_fonts )
  {
    ...
    // LOG("Into ft_open_face_internal function.", "DEBUG");

#ifndef FT_CONFIG_OPTION_MAC_FONTS
    FT_UNUSED( test_mac_fonts );
#endif

    // // LOG("undefine FT_CONFIG_OPTION_MAC_FONTS", "DEBUG");
...
#endif
    // // LOG("undefine FT_DEBUG_LEVEN_TRACE", "DEBUG");
    /* test for valid `library' delayed to `FT_Stream_New' */

    if ( !args )
      return FT_THROW( Invalid_Argument );
    // // LOG("Will into FT_BOOL fnction", "DEBUG");
    external_stream = FT_BOOL( ( args->flags & FT_OPEN_STREAM ) &&
                               args->stream                     );
    // // LOG("FT_BOOL done; Will into FT_Stream_New function", "DEBUG");
    /* create input stream */   
    {
    	char buf[256];
    ...
    {
      char buf[256];
      wsprintf(buf, "args = %04X:%04X, args->flags = %u, args->pathname = %s", 
      	(unsigned int)(((unsigned long)args) >> 16), 
      	(unsigned int)(((unsigned long)args) & 0xFFFF), 
      	args->flags, args->pathname ? args->pathname : "(null)");
      // LOG(buf, "ft_open_face_internal: args");
    }
	  memory = library->memory;                                      
    stream = FT_Stream_New16( library, args ); 
    // LOG("FT_Stream_New16 done", "DEBUG");                 
    
    if ( stream == NULL ){ 
    	// LOG("stream == NULL", "DEBUG");
      goto Fail3;         
    }  
    ...
    {
      // LOG("!aface && face_index >= 0 is true", "DEBUG");
      error = FT_THROW( Invalid_Argument );
      goto Fail3;
    }
    ...
    {
      // LOG("(args->flags&FT_OPEN_DRIVER)&&args->driver is true", "DEBUG");
      ...
      if ( FT_MODULE_IS_DRIVER( driver ) )
      {
        ...
        // LOG("B: before open_face", "DEBUG");
        error = open_face( driver, &stream, &external_stream, face_index,
                           num_params, params, &face );    
        {
        	char errBuf[64];
        	wsprintf(errBuf, "open_face returned error: 0x%04X (%d)", error, error);
        	// LOG("ft_open_face_internal", errBuf);
        }     
        // LOG("C: open_face done", "DEBUG");
        if ( !error )
          goto Success;
      }
      else
    ...
    {
      // LOG("(args->flags&FT_OPEN_DRIVER)&&args->driver is false", "DEBUG");
      ...
      for ( ; cur < limit; cur++ )
      {
        ...
        if ( FT_MODULE_IS_DRIVER( cur[0] ) )
        {
          FT_Int         num_params = 0;
          FT_Parameter*  params     = NULL;
          // LOG("FT_MODULE_IS_DRIVER( cur[0] ) is true", "DEBUG");

          driver = FT_DRIVER( cur[0] );

          if ( args->flags & FT_OPEN_PARAMS )
          {
            // LOG("args->flags & FT_OPEN_PARAMS is true", "DEBUG");
            num_params = args->num_params;
            params     = args->params;
          }
          // LOG("ft_open_face_internal", "will into open_face funciton");
          error = open_face( driver, &stream, &external_stream, face_index,
                             num_params, params, &face );       
        	// LOG("ft_open_face_internal", "open_face returned error: 0x%04X (%d)", error, error);
          if ( !error )
            goto Success;

#ifdef FT_CONFIG_OPTION_MAC_FONTS
		  // // LOG("define FT_CONFIG_OPTION_MAC_FONTS", "DEBUG");
          if ( test_mac_fonts                                           &&
  ...
  Success:                                                     
    // LOG("D: Success label reached", "DEBUG");
                    ...
                    const FT_Open_Args*  parameters )
  {
    ...
    error = FT_Stream_New( driver->root.library, memory, parameters, &stream );
    if ( error )
      goto Exit;
  ...
  FT_EXPORT_DEF( FT_Int ) __far __pascal
               ...
               FT_CMap       *acmap )
  {
    ...
    // LOG("FT_CMap_New", "Registered camp[%d] = %04X:%04X", 
    		// face->num_charmaps-1, FARPTR_SEG( cmap ), FARPTR_OFF( cmap ) );
  ...
  FT_EXPORT_DEF( FT_UInt ) __far __pascal
  ...
  FT_EXPORT_DEF( const char* ) __far __pascal
                            ...
                            FT_Render_Mode  render_mode )
  {
         ...
         slot->format == FT_GLYPH_FORMAT_OUTLINE    )
    {
      ...
      iterator.p  = NULL;   
      // LOG("FT_Render_Glyph_Internal", "will into FT_Get_Color_Glyph_Layer");
      have_layers = FT_Get_Color_Glyph_Layer( face,
                                              base_glyph,
                                              &glyph_index,
                                              &color_index,     
                                              &iterator );           
      // LOG("FT_Render_Glyph_Internal", "FT_Get_Color_Glyph_Layer returned: %u", have_layers);
      if ( have_layers )
      {
        // LOG("FT_Render_Glyph_Internal", "will into FT_New_GlyphSlot");
        error = FT_New_GlyphSlot( face, NULL );   
        // LOG("FT_Render_Glyph_Internal", " FT_NEW_GlyphSlot returned: %d", error);
        if ( !error )
        {
          TT_Face       ttface = (TT_Face)face;
          ...
          do
          {
            ...
            // LOG("FT_Render_Glyph_Internal", "FT_Load_Glyph returned: %d", error);
            if ( error )
              break;
    ...
    if ( slot->format == FT_GLYPH_FORMAT_OUTLINE )
    {                                     
      // LOG("FT_Render_Glyph_Internal", "slot->format==FT_GLYPH_FORMAT_OUTLINE is true");
      renderer = library->cur_renderer;
      node     = library->renderers.head;
    }
    else   
    {
      // LOG("FT_Render_Glyph_Internal", "slot->format==FT_GLYPH_FORMAT_OUTLINE is false");
      renderer = FT_Lookup_Renderer( library, slot->format, &node );
    }
    // LOG("FT_Render_Glyph_Internal", "first if-else done");
    error = FT_ERR( Cannot_Render_Glyph ); 
    // LOG("FT_Render_Glyph_Internal", "FT_ERR returned: %d", error);
    while ( renderer )
    {
      // LOG("FT_Render_Glyph_Internal", "while-ing, and will into renderer->render");      
      error = renderer->render( renderer, slot, render_mode, NULL );
      // LOG("FT_Render_Glyph_Internal", "renderer->render returned: %d", error);
      if ( !error                                   ||
    ...
    // LOG("FT_Render_Glyph_Internal", "while done");
...
#define FT_COMPONENT  objs
                   ...
                   FT_Render_Mode  render_mode )
  {
    ...
    // LOG("FT_Render_Glyph", "will return FT_Render_Glyph_Internal( library, slot, render_mode );");
  ...
  /* documentation is in ftmodapi.h */
    ...
    /* 安全读取 library->num_modules */
    hmemcpy( (void _huge *)&num_modules,
             (const void _huge *)&library->num_modules,
             sizeof( FT_UInt ) );
    // LOG("hmecpy done", "DEBUG"); 
    
    {
    char buf[256];
    int i;
    for ( i = 0; i < (int)num_modules && i < 7; i++ )
    {
        FT_Module tmp;
        hmemcpy( (void _huge *)&tmp,
                 (const void _huge *)&library->modules[i],
                 sizeof( FT_Module ) );
        wsprintf( buf, "modules[%d] = %04X:%04X", i,
            (unsigned)((unsigned long)tmp >> 16),
            (unsigned)((unsigned long)tmp & 0xFFFF) );
        // LOG(buf, "Array Dump" );
    }
}
    
    /* 查找同名模块 */
    /*for ( nn = 0; nn < num_modules; nn++ )
    {
        FT_Module        existing_module;
        FT_Module_Class  existing_clazz;

        hmemcpy( (void _huge *)&existing_module,
                 (const void _huge *)&library->modules[nn],
                 sizeof( FT_Module ) );

        hmemcpy( (void _huge *)&existing_clazz,
                 (const void _huge *)&existing_module->clazz,
                 sizeof( FT_Module_Class ) );

        {
      		char buf[256];
      		wsprintf(buf, "Module[%d]: %s", nn, existing_clazz.module_name);
      		// LOG("FT_Add_Module", buf);
    		}                                                                       
        
        if ( ft_strcmp( existing_clazz.module_name, clazz->module_name ) == 0 )
        {
            if ( clazz->module_version <= existing_clazz.module_version )
                return FT_THROW( Lower_Module_Version );

            FT_Remove_Module( library, existing_module );
            break;
        }
    }  */
    ...
    if ( num_modules >= FT_MAX_MODULES )
    {
        error = FT_THROW( Too_Many_Drivers );
        goto Exit;
    }

    // LOG("DEBUG", "Allocating module...");

    /* 用 GlobalAlloc(GPTR) 分配模块对象，返回完整的 far 指针 */
    {
        HGLOBAL hModule;
        void FAR* lpModule;
        
        hModule = GlobalAlloc( GPTR, clazz->module_size );
        if ( !hModule )
        ...
        module = (FT_Module)lpModule;
        module_val = (unsigned long)hModule;
    }                                       

    // LOG("DEBUG", "Allocation done. Assigning clazz...");
    hmemcpy( (void __huge *)&module->clazz, 
    					(const void __huge *)&clazz, 
    					sizeof( FT_Module_Class* ) );
    
    // LOG("DEBUG", "Assigning library...");
    hmemcpy( (void __far *)&module->library, 
    					(const void __huge *)&library, 
    					sizeof( FT_Library ) );

    // LOG("DEBUG", "Assigning memory...");
    hmemcpy( (void __far *)&module->memory, 
    					(const void __huge *)&memory, 
    					sizeof( FT_Memory ) );

    /* 后续渲染器、自动提示器、驱动初始化、模块初始化等，同样插入弹窗 */
    // LOG("All base assignments done. Checking renderer...", "DEBUG");
    if ( FT_MODULE_IS_RENDERER( module ) )
    {
        // LOG("Calling ft_add_renderer...", "DEBUG");
    ...
    if ( FT_MODULE_IS_DRIVER( module ) )
    {
        ...
        // LOG("Setting driver->clazz...", "DEBUG");
        driver->clazz = (FT_Driver_Class)module->clazz;  
        driver->root.library = library;
    }

    if ( clazz->module_init )
    {
        // LOG("Calling module_init...", "DEBUG");
        error = clazz->module_init( module );
        if ( error )
            goto Fail;
    }

    // LOG("Writing module to array...", "DEBUG");                            
    hmemcpy( (void __huge *)&library->modules[num_modules], 
    					(const void __huge *)&module, 
    					sizeof( FT_Module ) );
    /*num_modules++;
    hmemcpy( (void __huge *)&library->num_modules, 
    					(const void __huge *)&num_modules, 
    					sizeof( FT_UInt ) );*/                    
    (library->num_modules)++;
        
    // LOG("FT_Add_Module", "library->num_modules = %u", library->num_modules);
    // LOG("Module registered!", "DEBUG");

  Exit:
    return error;

  Fail:
    // LOG("Module registration failed!", "FAIL");
    if ( FT_MODULE_IS_RENDERER( module ) )
    {
        FT_Renderer       renderer = FT_RENDERER( module );
        FT_Renderer_Class rclazz;
        void*             raster;
        int               cond;

        hmemcpy( (void _huge *)&rclazz,
                 (const void _huge *)&renderer->clazz,
                 sizeof( FT_Renderer_Class ) );

        hmemcpy( (void _huge *)&raster,
                 (const void _huge *)&renderer->raster,
                 sizeof( void* ) );

        cond = ( rclazz.raster_class != NULL ) &&
               ( rclazz.glyph_format == FT_GLYPH_FORMAT_OUTLINE ) &&
               ( raster != NULL );

        if ( cond )
            rclazz.raster_class->raster_done( raster );
    }

    {
    	HGLOBAL hModule = (HGLOBAL)(unsigned long)module_val;
    	if ( hModule )
    	{
                 ...
                 const char*  module_name )
  {
    ...
    /*if ( IsBadReadPtr( library->modules[0]->clazz, sizeof( FT_Module_Class ) ) )
    	// LOG("FT_Get_Module", "library->modules[0]->clazz is BAD READ! ");
    else
    	// LOG("FT_Get_Module", "library->modules[0]->clazz can read");
    */
    cur   = library->modules;
    limit = cur + library->num_modules;
    ...
    for ( ; cur < limit; cur++, i++ )
    {
      // LOG("FT_Get_Module", "for-ing in %d", i);               
      // LOG("FT_Get_Module", "cur[0]->clazz->module_name = %s, module_name = %s", 
      		// cur[0]->clazz->module_name ? cur[0]->clazz->module_name : "(null)", 
      		// module_name ? module_name : "(null)");
      // LOG("FT_Get_Module", "check cur[0] read done. ");
      if ( _fstrcmp( cur[0]->clazz->module_name, module_name ) == 0 )
      {
        result = cur[0];
        break;
      }
                           ...
                           const char*  mod_name )
  {
    ...
    // LOG("will into FT_Get_Module", "DEBUG");  
    module = FT_Get_Module( library, mod_name );
    // LOG("FT_Get_Module end", "DEBUG");  
  ...
  FT_BASE_DEF( FT_Error )
  FT_New_Library_Internal( FT_Memory    memory,
                            FT_Library  *alibrary )
  {
    FT_Library  library = NULL;
    FT_Error    error;
    ...
    *alibrary = library;  /* alibrary 现在指向 DLL 栈上的变量，完全安全 */
    return FT_Err_Ok;
  }
