...
#define FT_STRUCTURE  TTC_HeaderRec
                  ...
                  FT_Parameter*  params )
  {
    ...
    // LOG("sfnt_init_face", "will into sfnt_open_font");
    error = sfnt_open_font( stream,
                            face,
                            &face_instance_index,
                            &woff2_num_faces );       
    // LOG("sfnt_init_face", "sfnt_open_font renturned: %d", error);
    if ( error )                                    
      return error;     
    ...
    if ( FT_STREAM_SEEK( face->ttc_header.offsets[face_index] ) )   
   	{
      // LOG("sfnt_init_face", "FT_STREAM_SEEK( face->ttc_header.offsets[face_index] ) is true");
      return error;
    }

    /* check whether we have a valid TrueType file */
    // LOG("sfnt_init_face", "will into sfnt->load_font_dir");
    error = sfnt->load_font_dir( face, stream );           
    // LOG("sfnt_init_face", "sfnt->load_font_dir returned: %d, face->format_tag = 0x%08lX", error, face->format_tag);
    if ( error )
      return error;
