...
#define FT_STRUCTURE  SFNT_HeaderRec
    ...
    // LOG("tt_face_load_font_dir", "Entered");
    ...
    HFILE file = (HFILE)((unsigned long)(stream->descriptor.pointer));
    int bytes_read;
    char logbuf[256];
    int i;

    /* ±£´æµ±Ç°ÎÄ¼þÖ¸ÕëÎ»ÖÃ */
    LONG saved_pos = _llseek( file, 0, 1 );  /* SEEK_CUR = 1 */

    /* ÒÆ¶¯µ½ÎÄ¼þ¿ªÍ· */
    _llseek( file, 0, 0 );  /* SEEK_SET = 0 */

    /* Ö±½Ó¶ÁÈ¡ 16 ×Ö½Ú */
    bytes_read = _lread( file, buf, 16 );

    /* »Ö¸´ÎÄ¼þÖ¸Õë */
    _llseek( file, saved_pos, 0 );
    ...
    if ( bytes_read == 16 )
    {
        ...
        {
            char hex[8];
            wsprintf( hex, "%02X ", buf[i] );
            lstrcat( logbuf, hex );
        }
        // LOG( logbuf, "tt_face_load_font_dir: first 16 bytes" );
    }
    else
    {
        wsprintf( logbuf, "tt_face_load_font_dir: _lread returned %d", bytes_read );
        // LOG( logbuf, "ERROR" );
    }
		}
    
    // LOG("tt_face_load_font_dir", "Before FT_READ_ULONG: stream->pos = %lu", stream->pos);
    ...
    {
    	char buf[64];
    	wsprintf(buf, "sfnt.format_tag = 0x%08lC", sfnt.format_tag);
    	// LOG("tt_face_load_font_dir", buf);
    }
    ...
    // LOG("tt_face_load_font_dir", "face->format_tag = 0x%08lX", face->format_tag);
