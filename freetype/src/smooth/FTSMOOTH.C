                    ...
                    const FT_Vector*  origin )
  {
    ...
    {                                                    
      ...
      else
      {
        ...
        params.source = outline;
        params.flags  = FT_RASTER_FLAG_AA;
        // LOG("ft_smooth_render", "error = %d, will into render->raster_render", error); 
        // LOG("ft_smooth_render", "render->raster_render ptr = %04X:%04X", FARPTR_SEG(render->raster_render), FARPTR_OFF(render->raster_render));
        /*if ( IsBadCodePtr( (FARPROC)render->raster_render ) )
        	LOG("ft_smooth_render", "render->raster_render is BAD CODE PTR!");
        else
        	LOG("ft_smooth_render", "render->raster_render is good code ptr");*/ 
        error = render->raster_render( render->raster, &params );  
        // LOG("ft_smooth_render", "error = %d, render->raster_render done", error);
      }
    }
    else
    {
    ...
    // LOG("ft_smooth_render", "error = %d, j", error);
  Exit:
    // LOG("ft_smooth_render", "into Exit:, and error = %d", error);
    if ( !error )
    {
