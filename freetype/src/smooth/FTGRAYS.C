 ...
 */
   ...
   *
   * B茅zier segments are flattened by splitting them until their deviation
   * from straight line becomes much smaller than a pixel.  Therefore, the
   * pixel coverage by a B茅zier curve is calculated approximately.  To
...
#define LEFT_SHIFT( a, b )  (FT_Int64)( (FT_UInt64)(a) << (b) )
  ...
  /*
   * For cubic B茅zier, binary splits are still faster than DDA
   ...
   *   segments and B茅zier arcs.  This function is also able to emit
...
#define SCALED( x )  ( (x) * ( 1L << shift ) - delta )
                      ...
                      const FT_Raster_Params*  params )
  {
    ...
    {
      static TCell  buffer[FT_MAX_GRAY_POOL];  /* NOT stack allocation!! */
