...
/* Win16 Open Watcom 2.0 fix: replace compound literals with static arrays */
static const gumbo_tagset kMathMLIntegrationPointTags = {
    TAG_MATHML(MI), TAG_MATHML(MO), TAG_MATHML(MN),
    TAG_MATHML(MS), TAG_MATHML(MTEXT)
};

static const gumbo_tagset kHTMLIntegrationPointTags = {
    TAG_SVG(FOREIGNOBJECT), TAG_SVG(DESC), TAG_SVG(TITLE)
};

/* Used in clear_stack_to_table_row_context, has_an_element_in_table_scope, etc. */
static const gumbo_tagset kTagSet_HTML_TR_TEMPLATE = {
    TAG(HTML), TAG(TR), TAG(TEMPLATE)
};

/* Used in clear_stack_to_table_context */
static const gumbo_tagset kTagSet_HTML_TABLE_TEMPLATE = {
    TAG(HTML), TAG(TABLE), TAG(TEMPLATE)
};

/* Used in clear_stack_to_table_body_context */
static const gumbo_tagset kTagSet_HTML_TBODY_TFOOT_THEAD_TEMPLATE = {
    TAG(HTML), TAG(TBODY), TAG(TFOOT), TAG(THEAD), TAG(TEMPLATE)
};

/* Used in get_appropriate_insertion_location (foster parenting) */
static const gumbo_tagset kTagSet_TABLE_TBODY_TFOOT_THEAD_TR = {
    TAG(TABLE), TAG(TBODY), TAG(TFOOT), TAG(THEAD), TAG(TR)
};

/* Used in has_an_element_in_scope, has_node_in_scope, and has_an_element_in_scope_with_tagname */
static const gumbo_tagset kTagSet_ScopeCommon = {
...
/* Used in has_an_element_in_list_scope (adds OL, UL) */
static const gumbo_tagset kTagSet_ListScope = {
    TAG(APPLET), TAG(CAPTION), TAG(HTML), TAG(TABLE), TAG(TD), TAG(TH),
    TAG(MARQUEE), TAG(OBJECT), TAG(TEMPLATE),
    TAG_MATHML(MI), TAG_MATHML(MO), TAG_MATHML(MN), TAG_MATHML(MS),
    TAG_MATHML(MTEXT), TAG_MATHML(ANNOTATION_XML),
...
/* Used in has_an_element_in_button_scope (adds BUTTON) */
static const gumbo_tagset kTagSet_ButtonScope = {
    TAG(APPLET), TAG(CAPTION), TAG(HTML), TAG(TABLE), TAG(TD), TAG(TH),
    TAG(MARQUEE), TAG(OBJECT), TAG(TEMPLATE),
    TAG_MATHML(MI), TAG_MATHML(MO), TAG_MATHML(MN), TAG_MATHML(MS),
    TAG_MATHML(MTEXT), TAG_MATHML(ANNOTATION_XML),
...
/* Used in generate_implied_end_tags (and similar) */
static const gumbo_tagset kTagSet_ImpliedEndTags = {
    TAG(DD), TAG(DT), TAG(LI), TAG(OPTION), TAG(OPTGROUP),
    TAG(P), TAG(RP), TAG(RB), TAG(RT), TAG(RTC)
};

/* Used in generate_all_implied_end_tags_thoroughly */
static const gumbo_tagset kTagSet_AllImpliedEndTags = {
...
/* Used in is_special_node */
static const gumbo_tagset kTagSet_Special = {
    TAG(ADDRESS), TAG(APPLET), TAG(AREA), TAG(ARTICLE), TAG(ASIDE),
    TAG(BASE), TAG(BASEFONT), TAG(BGSOUND), TAG(BLOCKQUOTE), TAG(BODY),
    TAG(BR), TAG(BUTTON), TAG(CAPTION), TAG(CENTER), TAG(COL),
    TAG(COLGROUP), TAG(DD), TAG(DETAILS), TAG(DIR), TAG(DIV), TAG(DL),
    TAG(DT), TAG(EMBED), TAG(FIELDSET), TAG(FIGCAPTION), TAG(FIGURE),
    TAG(FOOTER), TAG(FORM), TAG(FRAME), TAG(FRAMESET),
    TAG(H1), TAG(H2), TAG(H3), TAG(H4), TAG(H5), TAG(H6),
    TAG(HEAD), TAG(HEADER), TAG(HGROUP), TAG(HR), TAG(HTML),
    TAG(IFRAME), TAG(IMG), TAG(INPUT), TAG(LI), TAG(LINK), TAG(LISTING),
    ...
    TAG(STYLE), TAG(SUMMARY), TAG(TABLE), TAG(TBODY), TAG(TD),
    TAG(TEMPLATE), TAG(TEXTAREA), TAG(TFOOT), TAG(TH), TAG(THEAD),
    TAG(TITLE), TAG(TR), TAG(UL), TAG(WBR), TAG(XMP),
    TAG_MATHML(MI), TAG_MATHML(MO), TAG_MATHML(MN), TAG_MATHML(MS),
    TAG_MATHML(MTEXT), TAG_MATHML(ANNOTATION_XML),
    TAG_SVG(FOREIGNOBJECT), TAG_SVG(DESC)
};

/* Used in handle_in_head, handle_after_head, handle_in_body, etc. for <base> etc. */
static const gumbo_tagset kTagSet_BaseEtc = {
    TAG(BASE), TAG(BASEFONT), TAG(BGSOUND), TAG(LINK), TAG(META),
    TAG(NOFRAMES), TAG(SCRIPT), TAG(STYLE), TAG(TEMPLATE), TAG(TITLE)
};

/* Used in handle_in_head for <noframes> and <style> */
static const gumbo_tagset kTagSet_NoFramesStyle = {
    TAG(NOFRAMES), TAG(STYLE)
};

/* Used in handle_before_html, handle_before_head for end tags */
static const gumbo_tagset kTagSet_HeadBodyHtmlBr = {
    TAG(HEAD), TAG(BODY), TAG(HTML), TAG(BR)
};

/* Used in handle_in_body for <address>, <article>, etc. */
static const gumbo_tagset kTagSet_BlockElements = {
...
/* Used in handle_in_body for heading tags */
static const gumbo_tagset kTagSet_Headings = {
    TAG(H1), TAG(H2), TAG(H3), TAG(H4), TAG(H5), TAG(H6)
};

/* Used in handle_in_body for <pre> and <listing> */
static const gumbo_tagset kTagSet_PreListing = {
    TAG(PRE), TAG(LISTING)
};

/* Used in handle_in_body for <dd> and <dt> */
static const gumbo_tagset kTagSet_DdDt = {
    TAG(DD), TAG(DT)
};

/* Used in handle_in_body for <applet>, <marquee>, <object> */
static const gumbo_tagset kTagSet_AppletMarqueeObject = {
    TAG(APPLET), TAG(MARQUEE), TAG(OBJECT)
};

/* Used in handle_in_body for <area>, <br>, etc. */
static const gumbo_tagset kTagSet_AreaBrEmbedImg = {
    TAG(AREA), TAG(BR), TAG(EMBED), TAG(IMG), TAG(IMAGE), TAG(KEYGEN), TAG(WBR)
};

/* Used in handle_in_body for <param>, <source>, <track> */
static const gumbo_tagset kTagSet_ParamSourceTrack = {
    TAG(PARAM), TAG(SOURCE), TAG(TRACK)
};

/* Used in handle_in_body for <rb>, <rp>, <rt>, <rtc> */
static const gumbo_tagset kTagSet_RbRpRtRtc = {
    TAG(RB), TAG(RP), TAG(RT), TAG(RTC)
};

/* Used in handle_in_body for start tags that are parsed as errors */
static const gumbo_tagset kTagSet_TableChildStart = {
    TAG(CAPTION), TAG(COL), TAG(COLGROUP), TAG(FRAME), TAG(HEAD),
    TAG(TBODY), TAG(TD), TAG(TFOOT), TAG(TH), TAG(THEAD), TAG(TR)
};

/* Used in handle_in_table for <tbody>, <tfoot>, <thead>, <td>, <th>, <tr> */
static const gumbo_tagset kTagSet_TableBodyStart = {
    TAG(TBODY), TAG(TFOOT), TAG(THEAD), TAG(TD), TAG(TH), TAG(TR)
};

/* Used in handle_in_table for end tags that are ignored */
static const gumbo_tagset kTagSet_TableEndIgnored = {
    TAG(BODY), TAG(CAPTION), TAG(COL), TAG(COLGROUP), TAG(HTML),
    TAG(TBODY), TAG(TD), TAG(TFOOT), TAG(TH), TAG(THEAD), TAG(TR)
};

/* Used in handle_in_cell for end tags table, tbody, etc. */
static const gumbo_tagset kTagSet_TableEndCells = {
    TAG(TABLE), TAG(TBODY), TAG(TFOOT), TAG(THEAD), TAG(TR)
};

/* Used in handle_in_table_body for start tags caption, col, etc. */
static const gumbo_tagset kTagSet_TableBodyIgnoredStart = {
    TAG(CAPTION), TAG(COL), TAG(COLGROUP), TAG(TBODY), TAG(TFOOT), TAG(THEAD)
};

/* Used in handle_in_table_body for end tags body, caption, etc. */
static const gumbo_tagset kTagSet_TableBodyEndIgnored = {
    TAG(BODY), TAG(CAPTION), TAG(COL), TAG(TR), TAG(COLGROUP), TAG(HTML),
    TAG(TD), TAG(TH)
};

/* Used in handle_in_row for start tags caption, col, etc. */
static const gumbo_tagset kTagSet_RowIgnoredStart = {
    TAG(CAPTION), TAG(COL), TAG(COLGROUP), TAG(TBODY), TAG(TFOOT), TAG(THEAD), TAG(TR)
};

/* Used in handle_in_row for end tags body, caption, etc. */
static const gumbo_tagset kTagSet_RowEndIgnored = {
    TAG(BODY), TAG(CAPTION), TAG(COL), TAG(COLGROUP), TAG(HTML), TAG(TD), TAG(TH)
};

/* Used in handle_in_cell for end tags body, caption, etc. */
static const gumbo_tagset kTagSet_CellEndIgnored = {
    TAG(BODY), TAG(CAPTION), TAG(COL), TAG(COLGROUP), TAG(HTML)
};

/* Used in handle_in_cell for start tags caption, col, etc. */
static const gumbo_tagset kTagSet_CellStartIgnored = {
    TAG(CAPTION), TAG(COL), TAG(COLGROUP), TAG(TBODY), TAG(TD), TAG(TFOOT),
    TAG(TH), TAG(THEAD), TAG(TR)
};

/* Used in handle_in_template for start tags caption, colgroup, etc. */
static const gumbo_tagset kTagSet_TemplateTableStart = {
    TAG(CAPTION), TAG(COLGROUP), TAG(TBODY), TAG(TFOOT), TAG(THEAD)
};

/* Used in handle_in_template for start tags td, th */
static const gumbo_tagset kTagSet_TdTh = {
    TAG(TD), TAG(TH)
};

/* Used in has_open_element and has_an_element_in_specific_scope default */
static const gumbo_tagset kTagSet_HTML = {
    TAG(HTML)
};

/* Used in maybe_implicitly_close_list_tag */
static const gumbo_tagset kTagSet_AddressDivP = {
    TAG(ADDRESS), TAG(DIV), TAG(P)
};

/* Used in handle_after_head for end tags */
static const gumbo_tagset kTagSet_BodyHtmlBr = {
    TAG(BODY), TAG(HTML), TAG(BR)
};

/* Used in handle_in_body for end tags of block elements */
static const gumbo_tagset kTagSet_EndBlockTags = {
...
/* Used in handle_in_body for RT/RP exception */
static const gumbo_tagset kTagSet_RtRp = {
    TAG(RT), TAG(RP)
};

/* Used in handle_in_table for table body start */
static const gumbo_tagset kTagSet_TdThTr = {
    TAG(TD), TAG(TH), TAG(TR)
};

/* Used in handle_in_table for style/script/template */
static const gumbo_tagset kTagSet_ScriptStyleTemplate = {
    TAG(STYLE), TAG(SCRIPT), TAG(TEMPLATE)
};

/* Used in handle_in_caption for end tags that are ignored */
static const gumbo_tagset kTagSet_CaptionEndIgnored = {
    TAG(BODY), TAG(COL), TAG(COLGROUP), TAG(HTML), TAG(TBODY), TAG(TD),
    TAG(TFOOT), TAG(TH), TAG(THEAD), TAG(TR)
};

/* Used in handle_in_table_body and handle_in_row */
static const gumbo_tagset kTagSet_TBodyTFootTHead = {
    TAG(TBODY), TAG(TFOOT), TAG(THEAD)
};

/* Used in handle_in_row for TH/TD */
static const gumbo_tagset kTagSet_ThTd = {
    TAG(TH), TAG(TD)
};

/* Used in handle_in_foreign_content for start tags */
static const gumbo_tagset kTagSet_ForeignStartTags = {
...
/* Used in handle_in_foreign_content for end tags */
static const gumbo_tagset kTagSet_ForeignEndTags = {
    TAG(BR), TAG(P)
};

/* Used in handle_token for MGLYPH/MALIGNMARK */
static const gumbo_tagset kTagSet_MglyphMalignmark = {
    TAG(MGLYPH), TAG(MALIGNMARK)
};

/* handle_in_head: <base>, <basefont>, <bgsound>, <link> */
static const gumbo_tagset kTagSet_BaseLink = {
    TAG(BASE), TAG(BASEFONT), TAG(BGSOUND), TAG(LINK)
};

/* handle_in_head_noscript and handle_in_head: <basefont>, <bgsound>, <link>, <meta>, <noframes>, <style> */
static const gumbo_tagset kTagSet_BgSoundLinkMetaNoFramesStyle = {
    TAG(BASEFONT), TAG(BGSOUND), TAG(LINK), TAG(META), TAG(NOFRAMES), TAG(STYLE)
};

/* handle_in_head_noscript: <head>, <noscript> */
static const gumbo_tagset kTagSet_HeadNoscript = {
    TAG(HEAD), TAG(NOSCRIPT)
};

/* handle_after_head and others: <body>, <html> */
static const gumbo_tagset kTagSet_BodyHtml = {
    TAG(BODY), TAG(HTML)
};

/* handle_in_body EOF check ¨C large list */
static const gumbo_tagset kTagSet_EOFImpliedEnd = {
    TAG(DD), TAG(DT), TAG(LI), TAG(P), TAG(TBODY), TAG(TD), TAG(TFOOT),
    TAG(TH), TAG(THEAD), TAG(TR), TAG(BODY), TAG(HTML)
};

/* handle_in_body end formatting tags (A, B, BIG, ...) */
static const gumbo_tagset kTagSet_FormattingEndTags = {
    TAG(A), TAG(B), TAG(BIG), TAG(CODE), TAG(EM), TAG(FONT), TAG(I),
    TAG(NOBR), TAG(S), TAG(SMALL), TAG(STRIKE), TAG(STRONG), TAG(TT), TAG(U)
};

static GumboTag kHeadingTags[] = {
    GUMBO_TAG_H1, GUMBO_TAG_H2, GUMBO_TAG_H3,
    GUMBO_TAG_H4, GUMBO_TAG_H5, GUMBO_TAG_H6
};

static GumboTag kOptgroupTags[] = { GUMBO_TAG_OPTION, GUMBO_TAG_OPTGROUP };
...
const GumboOptions __export kGumboDefaultOptions = {&malloc_wrapper, &free_wrapper, NULL,
...
static bool is_mathml_integration_point(const GumboNode* node) {
  return node_tag_in_set(node, kMathMLIntegrationPointTags);
}

// http://www.whatwg.org/specs/web-apps/current-work/multipage/tree-construction.html#html-integration-point
static bool is_html_integration_point(const GumboNode* node) {
  return node_tag_in_set(node, kHTMLIntegrationPointTags) ||
...
InsertionLocation get_appropriate_insertion_location(
  ...
  if (!parser->_parser_state->_foster_parent_insertions ||
      !node_tag_in_set(retval.target, kTagSet_TableEndCells)) {
    return retval;
  }
...
void clear_stack_to_table_body_context(GumboParser* parser) {
  while (!node_tag_in_set(get_current_node(parser), kTagSet_HTML_TBODY_TFOOT_THEAD_TEMPLATE)) {
    pop_current_node(parser);
  }
}
...
static bool has_open_element(GumboParser* parser, GumboTag tag) {
  return has_an_element_in_specific_scope(
      parser, 1, &tag, false, kTagSet_HTML);
}
...
static bool has_an_element_in_scope(GumboParser* parser, GumboTag tag) {
  return has_an_element_in_specific_scope(parser, 1, &tag, false, kTagSet_ScopeCommon);
}
...
static bool has_node_in_scope(GumboParser* parser, const GumboNode* node) {
  ...
  for (int i = open_elements->length; --i >= 0;) {
    ...
    if (node_tag_in_set(current, kTagSet_ScopeCommon)) {
      return false;
    }
  }
  assert(false);
...
static bool has_an_element_in_scope_with_tagname(
  ...
  return has_an_element_in_specific_scope(parser, expected_len, expected, false, kTagSet_ScopeCommon);
}
...
static bool has_an_element_in_list_scope(GumboParser* parser, GumboTag tag) {
  return has_an_element_in_specific_scope(parser, 1, &tag, false, kTagSet_ListScope);
}
...
static bool has_an_element_in_button_scope(GumboParser* parser, GumboTag tag) {
  return has_an_element_in_specific_scope(parser, 1, &tag, false, kTagSet_ButtonScope);
}
...
static void generate_implied_end_tags(GumboParser* parser, GumboTag exception) {
  for (; node_tag_in_set(get_current_node(parser), kTagSet_ImpliedEndTags) &&
...
static void generate_all_implied_end_tags_thoroughly(GumboParser* parser) {
  for (
      ; node_tag_in_set(get_current_node(parser), kTagSet_AllImpliedEndTags);
      pop_current_node(parser))
    ;
}
...
static bool is_special_node(const GumboNode* node) {
  assert(node->type == GUMBO_NODE_ELEMENT || node->type == GUMBO_NODE_TEMPLATE);
  return node_tag_in_set(node, kTagSet_Special);
}
  ...
  for (int i = state->_open_elements.length; --i >= 0;) {
    ...
    bool is_list_tag =
        is_li ? node_html_tag_is(node, GUMBO_TAG_LI)
              : node_tag_in_set(node, kTagSet_DdDt);
    if (is_list_tag) {
    ...
    if (is_special_node(node) &&
        !node_tag_in_set(node, kTagSet_AddressDivP)) {
      return;
    }
  }
}
...
const char* GUMBO_16BIT_API gumbo_normalize_svg_tagname(const GumboStringPiece* tag) {
  ...
  for (unsigned int i = 0; i < 8; ++i) {
    ...
    for (int j = 0;;) {
      ...
      int formatting_index =
          gumbo_vector_index_of(&state->_active_formatting_elements, node);           gumbo_vector_index_of(&state->_active_formatting_elements, node);
...
static bool handle_in_head(GumboParser* parser, GumboToken* token) {
  if (token->type == GUMBO_TOKEN_WHITESPACE) {
  ...
  } else if (tag_in(token, kStartTag, kTagSet_BaseLink)) {
    insert_element_from_token(parser, token);
    pop_current_node(parser);
    ...
    return true;
                 ...
                 token, kStartTag, kTagSet_NoFramesStyle)) {
    ...
    return true;
  } else if (tag_in(token, kEndTag, kTagSet_BodyHtmlBr)) {
    pop_current_node(parser);
    set_insertion_mode(parser, GUMBO_INSERTION_MODE_AFTER_HEAD);
    parser->_parser_state->_reprocess_current_token = true;
    return true;
...
static bool handle_in_head_noscript(GumboParser* parser, GumboToken* token) {
  if (token->type == GUMBO_TOKEN_DOCTYPE) {
    ...
    return true;
             ...
             tag_in(token, kStartTag, kTagSet_BgSoundLinkMetaNoFramesStyle)) {
    return handle_in_head(parser, token);
  } else if (tag_in(
                 token, kStartTag, kTagSet_HeadNoscript) ||
             (token->type == GUMBO_TOKEN_END_TAG &&
...
static bool handle_after_head(GumboParser* parser, GumboToken* token) {
  ...
  if (token->type == GUMBO_TOKEN_WHITESPACE) {
    ...
    return true;
  } else if (tag_in(token, kStartTag, kTagSet_BaseEtc)) {
    parser_add_parse_error(parser, token);
...
static bool handle_in_body(GumboParser* parser, GumboToken* token) {
  ...
  if (token->type == GUMBO_TOKEN_NULL) {
    ...
    for (unsigned int i = 0; i < state->_open_elements.length; ++i) {
      if (!node_tag_in_set(state->_open_elements.data[i], kTagSet_EOFImpliedEnd)) {
        parser_add_parse_error(parser, token);
      }
    }
    ...
    return true;
  } else if (tag_in(token, kEndTag, kTagSet_BodyHtml)) {
    ...
    return success;
  } else if (tag_in(token, kStartTag, kTagSet_BlockElements))
  {
    bool result = maybe_implicitly_close_p_tag(parser, token);
    insert_element_from_token(parser, token);
    return result;
  } else if (tag_in(token, kStartTag, kTagSet_Headings)) {
    bool result = maybe_implicitly_close_p_tag(parser, token);
    if (node_tag_in_set(
            get_current_node(parser), kTagSet_Headings)) {
      parser_add_parse_error(parser, token);
      pop_current_node(parser);
      result = false;
    }
    insert_element_from_token(parser, token);
    return result;
  } else if (tag_in(token, kStartTag, kTagSet_PreListing)) {
    bool result = maybe_implicitly_close_p_tag(parser, token);
    insert_element_from_token(parser, token);
    ...
    insert_element_from_token(parser, token);
    return result;
  } else if (tag_in(token, kStartTag, kTagSet_DdDt)) {
    ...
    state->_frameset_ok = false;
    return true;
  } else if (tag_in(token, kEndTag, kTagSet_EndBlockTags))
  {
  ...
  } else if (tag_in(token, kEndTag, kTagSet_DdDt)) {
  ...
  } else if (tag_in(token, kEndTag, kTagSet_Headings)) {
    if (!has_an_element_in_scope_with_tagname(parser, 6, kHeadingTags)) {
      ...
      } while (!node_tag_in_set(current_node, kTagSet_Headings));
      return success;
    }
    ...
    return success;
  } else if (tag_in(token, kStartTag, kTagSet_ForeignStartTags)) {
    reconstruct_active_formatting_elements(parser);
    add_formatting_element(parser, insert_element_from_token(parser, token));
    return true;
    ...
    return result;
  } else if (tag_in(token, kEndTag, kTagSet_FormattingEndTags)) {
    return adoption_agency_algorithm(parser, token, token->v.end_tag);
  } else if (tag_in(token, kStartTag, kTagSet_AppletMarqueeObject)) {
    reconstruct_active_formatting_elements(parser);
    insert_element_from_token(parser, token);
    add_formatting_element(parser, &kActiveFormattingScopeMarker);
    set_frameset_not_ok(parser);
    return true;
  } else if (tag_in(token, kEndTag, kTagSet_AppletMarqueeObject)) {
    GumboTag token_tag = token->v.end_tag;
    ...
    return true;
  } else if (tag_in(token, kStartTag, kTagSet_AreaBrEmbedImg)) {
    bool success = true;
    ...
    if (has_an_element_in_scope(parser, GUMBO_TAG_SELECT)) {
      generate_implied_end_tags(parser, GUMBO_TAG_LAST);
      if (has_an_element_in_scope_with_tagname(parser, 2, kOptgroupTags)) {
        parser_add_parse_error(parser, token);
      }
    }
    insert_element_from_token(parser, token);
    ...
    bool success = true;
    GumboTag exception =
        tag_in(token, kStartTag, kTagSet_RtRp)
            ? GUMBO_TAG_RTC
            : GUMBO_TAG_LAST;
    ...
    return true;
  } else if (tag_in(token, kStartTag, kTagSet_TableChildStart)) {
    parser_add_parse_error(parser, token);
    ignore_token(parser);
    return false;
...
static bool handle_in_table(GumboParser* parser, GumboToken* token) {
  ...
  if (token->type == GUMBO_TOKEN_CHARACTER ||
    ...
    set_insertion_mode(parser, GUMBO_INSERTION_MODE_IN_COLUMN_GROUP);
    return true;
  } else if (tag_in(token, kStartTag, kTagSet_TableBodyStart)) {
    clear_stack_to_table_context(parser);
    set_insertion_mode(parser, GUMBO_INSERTION_MODE_IN_TABLE_BODY);
    if (tag_in(token, kStartTag, kTagSet_TdThTr)) {
      insert_element_of_tag_type(
    ...
    return true;
  } else if (tag_in(token, kEndTag, kTagSet_TableEndIgnored)) {
    parser_add_parse_error(parser, token);
    ignore_token(parser);
    return false;
  } else if (tag_in(token, kStartTag, kTagSet_ScriptStyleTemplate) ||
             (tag_is(token, kEndTag, GUMBO_TAG_TEMPLATE))) {
    return handle_in_head(parser, token);
  } else if (tag_is(token, kStartTag, GUMBO_TAG_INPUT) &&
             attribute_matches(
...
static bool handle_in_caption(GumboParser* parser, GumboToken* token) {
  if (tag_is(token, kEndTag, GUMBO_TAG_CAPTION)) {
  ...
  } else if (tag_in(token, kStartTag, kTagSet_CellStartIgnored) ||
    ...
    parser->_parser_state->_reprocess_current_token = true;
    return true;
  } else if (tag_in(token, kEndTag, kTagSet_CaptionEndIgnored)) {
    parser_add_parse_error(parser, token);
    ignore_token(parser);
    return false;
  } else {
...
static bool handle_in_table_body(GumboParser* parser, GumboToken* token) {
  if (tag_is(token, kStartTag, GUMBO_TAG_TR)) {
    ...
    set_insertion_mode(parser, GUMBO_INSERTION_MODE_IN_ROW);
    return true;
  } else if (tag_in(token, kStartTag, kTagSet_TdTh)) {
    parser_add_parse_error(parser, token);
    clear_stack_to_table_body_context(parser);
    ...
    set_insertion_mode(parser, GUMBO_INSERTION_MODE_IN_TABLE);
    return true;
  } else if (tag_in(token, kStartTag, kTagSet_TableBodyIgnoredStart) ||
             tag_is(token, kEndTag, GUMBO_TAG_TABLE)) {
    ...
    parser->_parser_state->_reprocess_current_token = true;
    return true;
  } else if (tag_in(token, kEndTag, kTagSet_TableBodyEndIgnored)) {
    parser_add_parse_error(parser, token);
    ignore_token(parser);
    return false;
  } else {
...
static bool handle_in_row(GumboParser* parser, GumboToken* token) {
  if (tag_in(token, kStartTag, kTagSet_ThTd)) {
  ...
  } else if (tag_in(token, kStartTag, kTagSet_RowIgnoredStart) ||
             tag_is(token, kEndTag, GUMBO_TAG_TABLE)) {
    if (!has_an_element_in_table_scope(parser, GUMBO_TAG_TR)) {
      parser_add_parse_error(parser, token);
      ignore_token(parser);
  ...
  } else if (tag_in(token, kEndTag, kTagSet_RowEndIgnored)) {
    parser_add_parse_error(parser, token);
    ignore_token(parser);
    return false;
  } else {
...
static bool handle_in_cell(GumboParser* parser, GumboToken* token) {
  if (tag_in(token, kEndTag, kTagSet_TdTh)) {
    GumboTag token_tag = token->v.end_tag;
    if (!has_an_element_in_table_scope(parser, token_tag)) {
      parser_add_parse_error(parser, token);
      ignore_token(parser);
  ...
  } else if (tag_in(token, kEndTag, kTagSet_CellEndIgnored)) {
    parser_add_parse_error(parser, token);
    ignore_token(parser);
    return false;
  } else if (tag_in(token, kEndTag, kTagSet_TABLE_TBODY_TFOOT_THEAD_TR)) {
    if (!has_an_element_in_table_scope(parser, token->v.end_tag)) {
      parser_add_parse_error(parser, token);
      ignore_token(parser);
      return false;
...
static bool handle_in_template(GumboParser* parser, GumboToken* token) {
  ...
  if (token->type == GUMBO_TOKEN_WHITESPACE ||
    ...
    return handle_in_head(parser, token);
                 ...
                 token, kStartTag, kTagSet_TemplateTableStart)) {
    pop_template_insertion_mode(parser);
...
static bool handle_in_foreign_content(GumboParser* parser, GumboToken* token) {
  ...
  if (tag_in(token, kStartTag, kTagSet_ForeignStartTags)
      || tag_in(token, kEndTag, kTagSet_ForeignEndTags)
  ...
  if (token->type == GUMBO_TOKEN_START_TAG) {
    ...
    for (--i; i > 0;) {
      ...
      gumbo_tag_from_original_text(&node_tagname);       gumbo_tag_from_original_text(&node_tagname);
    }
...
GumboOutput* GUMBO_16BIT_API gumbo_parse(const char* buffer) {
  return gumbo_parse_with_options(
      &kGumboDefaultOptions, buffer, strlen(buffer));
}

GumboOutput* GUMBO_16BIT_API gumbo_parse_with_options(
...
void GUMBO_16BIT_API gumbo_destroy_output(const GumboOptions* options, GumboOutput* output) {
  // Need a dummy GumboParser because the allocator comes along with the
  // options object.
  GumboParser parser;
  parser._options = options;
