...
#define CHARSET_LE "@\0\0\0c\0\0\0h\0\0\0a\0\0\0r\0\0\0s\0\0\0e\0\0\0t\0\0\0 \0\0\0\"\0\0\0"
        ...
        if (memcmp(data, CHARSET_LE, SLEN(CHARSET_LE)) == 0) {
                ...
                for (end = start; end < data + len - 4; end += 4) {
                        uint32_t c = end[0] | ((uint32_t)end[1] << 8) |
                        ...
                        /* Reached the end? */
                        if (c == '"' && end < data + len - 8) {
                                uint32_t d = end[4] | ((uint32_t)end[5] << 8) |
                                    ((uint32_t)end[6] << 16) | ((uint32_t)end[7] << 24);
                ...
                /* Look for "; at end of charset declaration */
                for (end = start; end < data + len - 4; end += 4) {
                        uint32_t c = end[3] | ((uint32_t)end[2] << 8) |
                                     ((uint32_t)end[1] << 16) | ((uint32_t)end[0] << 24);
                        ...
                        /* Reached the end? */
                        if (c == '"' && end < data + len - 8) {
                                uint32_t d = end[7] | ((uint32_t)end[6] << 8) |
                                    ((uint32_t)end[5] << 16) | ((uint32_t)end[4] << 24);
