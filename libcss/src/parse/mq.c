                ...
                css_mq_cond_or_feature **cond_or_feature)
{
        ...
        if (tokenIsChar(token, '(') || (token->type == CSS_TOKEN_IDENT &&
                ...
                if (error == CSS_OK) {
                        ...
                        result = css_malloc(sizeof(*result));
                        if (result == NULL) {
                ...
                bool permit_or, css_mq_cond **cond)
{
        ...
        if (tokenIsChar(token, '(') == false) {
                ...
                result->negate = 1;
                result->parts = css_malloc(sizeof(*result->parts));
        ...
        while (token != NULL && tokenIsChar(token, ')') == false &&
                ...
                parts = css_realloc(result->parts,
                ...
                css_mq_query **not_all_out)
{
        ...
        not_all = css_calloc(1, sizeof(*not_all));
        if (not_all == NULL) {
                return CSS_NOMEM;
        }
