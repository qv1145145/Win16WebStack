...
struct keyval *get_keyval(char **pos)
{
        ...
        nkeyval = css_calloc(1, sizeof(struct keyval) + kvlen + 1);
        if (nkeyval == NULL) {
        ...
        for (int i = 0; i < IDENT.count; i++) {
                if (IDENT.item[i] != &ident_inherit && IDENT.item[i] != &ident_initial &&
                    IDENT.item[i] != &ident_revert && IDENT.item[i] != &ident_unset) {
                        css_free(IDENT.item[i]);
                }
        }
        for (int i = 0; i < URI.count; i++) css_free(URI.item[i]);
        for (int i = 0; i < WRAP.count; i++) css_free(WRAP.item[i]);
        for (int i = 0; i < NUMBER.count; i++) css_free(NUMBER.item[i]);
        for (int i = 0; i < COLOR.count; i++) css_free(COLOR.item[i]);
        for (int i = 0; i < LENGTH_UNIT.count; i++) css_free(LENGTH_UNIT.item[i]);
        for (int i = 0; i < IDENT_LIST.count; i++) css_free(IDENT_LIST.item[i]);
        for (int i = 0; i < CALC.count; i++) css_free(CALC.item[i]);
        css_free(descriptor);
        if (outputf != stdout) fclose(outputf);
