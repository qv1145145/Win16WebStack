...
css_error css__stylesheet_string_add(css_stylesheet *sheet, lwc_string *string, uint32_t *string_number)
{
        ...
        if (sheet->string_vector_c >= sheet->string_vector_l) {
                ...
                new_vector = css_realloc(sheet->string_vector,
 ...
 *         CSS_NOMEM on memory exhaustion
 */
css_error LIBCSS_API css_stylesheet_create(const css_stylesheet_params *params,
                css_stylesheet **stylesheet)
{
        ...
        sheet = css_calloc(1, sizeof(css_stylesheet));
 ...
 */
css_error LIBCSS_API css_stylesheet_destroy(css_stylesheet *sheet)
{
        uint32_t index;
        css_rule *r, *s;
 ...
 * \return CSS_OK on success, appropriate error otherwise
 */
css_error LIBCSS_API css_stylesheet_append_data(css_stylesheet *sheet,
 ...
 */
css_error LIBCSS_API css_stylesheet_data_done(css_stylesheet *sheet)
{
        const css_rule *r;
        css_error error;
 ...
 */
css_error LIBCSS_API css_stylesheet_next_pending_import(css_stylesheet *parent,
                lwc_string **url)
{
        const css_rule *r;
 ...
 */
css_error LIBCSS_API css_stylesheet_register_import(css_stylesheet *parent,
 ...
 * \return CSS_OK on success, appropriate error otherwise
 */
css_error LIBCSS_API css_stylesheet_get_language_level(css_stylesheet *sheet,
 ...
 * \return CSS_OK on success, appropriate error otherwise
 */
css_error LIBCSS_API css_stylesheet_get_url(css_stylesheet *sheet, const char **url)
{
 ...
 * \return CSS_Ok on success, appropriate error otherwise
 */
css_error LIBCSS_API css_stylesheet_get_title(css_stylesheet *sheet, const char **title)
{
 ...
 * \return CSS_OK on success, appropriate error otherwise
 */
css_error LIBCSS_API css_stylesheet_quirks_allowed(css_stylesheet *sheet, bool *allowed)
{
 ...
 * \return CSS_OK on success, appropriate error otherwise
 */
css_error LIBCSS_API css_stylesheet_used_quirks(css_stylesheet *sheet, bool *quirks)
{
 ...
 * \return CSS_OK on success, appropriate error otherwise
 */
css_error LIBCSS_API css_stylesheet_get_disabled(css_stylesheet *sheet, bool *disabled)
{
 ...
 * \return CSS_OK on success, appropriate error otherwise
 */
css_error LIBCSS_API css_stylesheet_set_disabled(css_stylesheet *sheet, bool disabled)
{
        if (sheet == NULL)
                return CSS_BADPARM;
 ...
 */
css_error LIBCSS_API css_stylesheet_size(css_stylesheet *sheet, size_t *size)
{
        size_t bytes = 0;
        css_error error;
 ...
 * 16 seems to be a good medium between wastage and css_reallocs.
 */
...
css_error css__stylesheet_style_create(css_stylesheet *sheet, css_style **style)
{
        ...
        s = css_malloc(sizeof(css_style));
        if (s == NULL)
                return CSS_NOMEM;

        s->bytecode = css_malloc(sizeof(css_code_t) * CSS_STYLE_DEFAULT_SIZE);

        if (s->bytecode == NULL) {
                css_free(s); /* do not leak */
...
css_error css__stylesheet_merge_style(css_style *target, css_style *style)
{
        ...
        if (newcode_len > target->allocated) {
                ...
                newcode_len &= ~(CSS_STYLE_DEFAULT_SIZE - 1);
                newcode = css_realloc(target->bytecode,
...
css_error css__stylesheet_style_append(css_style *style, css_code_t css_code)
{
        ...
        if (style->allocated == style->used) {
                ...
                newcode = css_realloc(style->bytecode,
...
css_error css__stylesheet_style_destroy(css_style *style)
{
        ...
        if (sheet->cached_style == NULL) {
                ...
                css_free(sheet->cached_style->bytecode);
                css_free(sheet->cached_style);
                sheet->cached_style = style;
                style->used = 0;
        } else {
                css_free(style->bytecode);
                css_free(style);
        }
                ...
                css_qname *qname, css_selector **selector)
{
        ...
        sel = css_malloc(sizeof(css_selector));
        if (sel == NULL)
                return CSS_NOMEM;
                ...
                css_selector **parent, const css_selector_detail *detail)
{
        ...
        temp = css_realloc((*parent), sizeof(css_selector) +
