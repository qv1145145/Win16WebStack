...
#include "select/selet_font_face.h"
...
static css_error css__create_node_data(struct css_node_data **node_data)
{
        ...
        nd = css_calloc(1, sizeof(struct css_node_data));
        if (nd == NULL) {
                return CSS_NOMEM;
        }
...
css_error LIBCSS_API css_libcss_node_data_handler(css_select_handler *handler,
 ...
 */
css_error LIBCSS_API css_select_ctx_create(css_select_ctx **result)
{
        css_select_ctx *c;
        css_error error;
        ...
        c = css_calloc(1, sizeof(css_select_ctx));
        if (c == NULL)
                return CSS_NOMEM;
 ...
 * \return CSS_OK on success, appropriate error otherwise
 */
css_error LIBCSS_API css_select_ctx_destroy(css_select_ctx *ctx)
{
        if (ctx == NULL)
                return CSS_BADPARM;
 ...
 * \return CSS_OK on success, appropriate error otherwise
 */
css_error LIBCSS_API css_select_ctx_append_sheet(css_select_ctx *ctx,
                const css_stylesheet *sheet, css_origin origin,
                const char *media)
{
 ...
 * \return CSS_OK on success, appropriate error otherwise
 */
css_error LIBCSS_API css_select_ctx_insert_sheet(css_select_ctx *ctx,
 ...
 * \return CSS_OK on success, appropriate error otherwise
 */
css_error LIBCSS_API css_select_ctx_remove_sheet(css_select_ctx *ctx,
                const css_stylesheet *sheet)
{
        uint32_t index;
 ...
 * \return CSS_OK on success, appropriate error otherwise
 */
css_error LIBCSS_API css_select_ctx_count_sheets(css_select_ctx *ctx, uint32_t *count)
{
 ...
 * \return CSS_OK on success, appropriate error otherwise
 */
css_error LIBCSS_API css_select_ctx_get_sheet(css_select_ctx *ctx, uint32_t index,
                const css_stylesheet **sheet)
{
        if (ctx == NULL || sheet == NULL)
                return CSS_BADPARM;
 ...
 * \return CSS_OK on success, appropriate error otherwise.
 */
css_error LIBCSS_API css_select_default_style(css_select_ctx *ctx,
                css_select_handler *handler, void *pw,
                css_computed_style **style)
{
        css_error error;
 ...
 *                      Note: if there's no parent, the caller must css_free
                ...
                css_bloom **parent_bloom)
{
        ...
        if (bloom == NULL) {
                ...
                if (parent != NULL) {
                         ...
                         */
                        bloom = css_malloc(sizeof(css_bloom) * CSS_BLOOM_SIZE);
                        if (bloom == NULL) {
                                return CSS_NOMEM;
                        }
                ...
                css_bloom **node_bloom, css_select_state *state)
{
        ...
        *node_bloom = NULL;
        ...
        bloom = css_calloc(CSS_BLOOM_SIZE, sizeof(css_bloom));
        if (bloom == NULL) {
                return CSS_NOMEM;
        }
                ...
                void *pw)
{
        ...
        state->results = css_calloc(1, sizeof(css_select_results));
        if (state->results == NULL) {
                return CSS_NOMEM;
        }
 ...
 */
css_error LIBCSS_API css_select_style(css_select_ctx *ctx, void *node,
                const css_unit_ctx *unit_ctx,
                ...
                css_select_results **result)
{
        ...
        if (ctx->uses_revert ||
                ...
                state.revert = css_calloc(CSS_ORIGIN_AUTHOR, sizeof(*state.revert));
                if (state.revert == NULL) {
                        error = CSS_NOMEM;
                        goto cleanup;
                }
...
complete:
         ...
         * css_freed when the selection state is finalised */
 ...
 * \return CSS_OK on success, appropriate error otherwise
 */
css_error LIBCSS_API css_select_results_destroy(css_select_results *results)
{
        uint32_t i;
 ...
 * \return CSS_OK on success, appropriate error otherwise.
 */
css_error LIBCSS_API css_select_font_faces(css_select_ctx *ctx,
                const css_media *media,
                const css_unit_ctx *unit_ctx,
                lwc_string *font_family,
                css_select_font_faces_results **result)
{
        ...
        if (n_font_faces > 0) {
                ...
                results = css_malloc(sizeof(css_select_font_faces_results));
                if (results == NULL) {
                        error = CSS_NOMEM;
                        goto cleanup;
                }

                results->font_faces = css_malloc(
...
cleanup:
        if (state.ua_font_faces.count != 0)
                css_free(state.ua_font_faces.font_faces);

        if (state.user_font_faces.count != 0)
                css_free(state.user_font_faces.font_faces);

        if (state.author_font_faces.count != 0)
                css_free(state.author_font_faces.font_faces);
 ...
 * \return CSS_OK on success, appropriate error otherwise
 */
css_error LIBCSS_API css_select_font_faces_results_destroy(
                css_select_font_faces_results *results)
{
        if (results == NULL)
                return CSS_BADPARM;
                ...
                const css_select_strings *str)
{
        if (mq_rule_good_for_media((const css_rule *) rule,
                ...
                if (lwc_string_isequal(
                        ...
                        new_faces = css_realloc(faces->font_faces, new_size);
                        if (new_faces == NULL) {
                                return CSS_NOMEM;
                        }
                ...
                const css_stylesheet *sheet, css_select_state *state)
{
        ...
        if (state->classes != NULL && n_classes > 0) {
                ...
                class_selectors = css_malloc(n_classes * sizeof(css_selector **));
