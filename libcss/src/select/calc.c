...
{
        *out = css_calloc(1, sizeof(css_calculator));
        if (*out == NULL) {
                return CSS_NOMEM;
        }
        ...
        (*out)->stack =
            css_calloc(DEFAULT_STACK_SIZE, sizeof(css_calculator_stack_entry));
        if ((*out)->stack == NULL) {
#ifndef NDEBUG
                                      ...
                                      css_fixed value)
{
        if (calc->stack_ptr == calc->stack_alloc) {
                css_calculator_stack_entry *newstack =
                    css_realloc(calc->stack, sizeof(css_calculator_stack_entry) *
                                             2 * calc->stack_alloc);
                if (newstack == NULL) {
                        return CSS_NOMEM;
                }
