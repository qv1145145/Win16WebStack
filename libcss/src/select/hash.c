...
css_error css__selector_hash_create(css_selector_hash **hash)
{
        ...
        h = css_calloc(1, sizeof(css_selector_hash));
        if (h == NULL)
                return CSS_NOMEM;

        /* Element hash */
        h->elements.slots = css_calloc(DEFAULT_SLOTS, sizeof(hash_entry));
        ...
        /* Class hash */
        h->classes.slots = css_calloc(DEFAULT_SLOTS, sizeof(hash_entry));
        if (h->classes.slots == NULL) {
                css_free(h->elements.slots);
                css_free(h);
                return CSS_NOMEM;
        }
        ...
        /* ID hash */
        h->ids.slots = css_calloc(DEFAULT_SLOTS, sizeof(hash_entry));
        if (h->ids.slots == NULL) {
                css_free(h->classes.slots);
                css_free(h->elements.slots);
                css_free(h);
                return CSS_NOMEM;
        }
        ...
        /* Universal chain head already initiliased by css_calloc of `h`. */
                ...
                const css_selector *selector)
{
        if (head->sel == NULL) {
                ...
                hash_entry *entry = css_malloc(sizeof(hash_entry));
                if (entry == NULL)
                        return CSS_NOMEM;
